<?php 
    ...
    if(isset($_GET['obtener_datos_compra_especifica_con_estatus_disponibles'])){

        $numero_factura = $_GET['obtener_datos_compra_especifica_con_estatus_disponibles'];

         //Validamos el numero de factura
         if(!validar_numero_factura($numero_factura)){
            echo json_encode(['success' => ['Formato de número de factura incorrecto','search_compra_disponible']]);
            return;
        }

        try{
            $datos = mysqli_query($conexion, "SELECT * FROM compra WHERE numero_factura = '$numero_factura' AND seleccionado = 0");
            if(mysqli_num_rows($datos) > 0){
                $datos = mysqli_fetch_all($datos,MYSQLI_ASSOC);
                echo json_encode(["success" => $datos]);
                exit();
    ...
    //Obtener compras (Todas) con estatus disponible
    ...
    if(isset($_GET['recibir_datos_compra'])){
        ...
        if(!validarNumeros($precio_compra)){
