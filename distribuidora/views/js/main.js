const delete_sessiones = "http://localhost/ProyectoIV-II/distribuidora/controllers/delete_sessiones.php";
...
$(document).ready(function(){
    ...
    $('.btn-exit').on('click', function(){
		...
		async function(isConfirm) {
		  	if (isConfirm) {
				try {
					const res = await fetch(`${delete_sessiones}?cerrarSesion=1`);
					const data = await(res.json());
					let dataRemove = localStorage.removeItem('datos_user')
					if(data['data'] != null){
