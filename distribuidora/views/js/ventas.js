...
const d = document,
$venta_productos = d.querySelector('#venta_productos'),
...
$btn_registrar_venta = d.querySelector('#btn-registrar-venta');
let data_productos;
...
const show_lista_productos = (lista_productos_seleccionados) => {
    ...
    if(lista_productos_seleccionados.length > 0){
        ...
        lista_productos_seleccionados.forEach(elemento => {
            ...
            $items_lista_productos.querySelector('.iva').textContent = elemento['grabado_excento'] == 1 ? Number(elemento['precio']) * 0.16 : "-"
...
const addProduct_in_array_product = (id_producto, cantidad) => {
    ...
    let nombre_producto = `${datos_producto[0]['nombre_producto']} ${data_productos[0]['contenido_neto']}L`;
...
const save_venta_productos = async(datos,url) => {
    ...
    try {
        const res = await fetch(`${url}?recibir_datos_venta=1`,body);
        if(!res.ok) throw {status : res.status, statusText : res.statusText};
        const data = await res.json();
        return data;
...
const comprobar_disponibilidad_producto = async (productos) => {
    ...
    for (let i = 0; i < productos.length; i++) {
        if(Number(productos[i].cantidad) > (Number(data_productos[i].cantidad_disponible) - Number(data_productos[i].cantidad_apartada == null ? 0 : data_productos[i].cantidad_apartada))){
